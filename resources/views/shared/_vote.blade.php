...
<div class="d-flex flex-column vote-controls">
    ...
    <form id="up-vote-{{$name}}-{{ $model->id }}" action="{{$model->id }}/vote" method="POST" style="display:none;">
        @csrf
    ...
    <form id="down-vote-{{$name}}-{{ $model->id }}" action="{{$model->id }}/vote" method="POST" style="display:none;">
        @csrf
        <input type="hidden" name="vote" value="-1">
    </form>
    @if ($model instanceof App\Question)
